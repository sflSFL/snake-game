...
public class Board {
    ...
    private String[] foodTypes = {
            "green_icing_green_sprinkles", "blue_icing", "chocolate_icing", "chocolate_icing_chocolate_drizzle",
            "dark_red_icing",
            "orange_icing_chocolate_shaving", "pink_icing_sprinkles", "orange_icing_chocolate_shaving",
            "pink_icing_sprinkles",
    ...
    private void initBoard(int width, int height) {
        ...
        for (int rowGrass = 0; rowGrass < cells.length; rowGrass++) {
            for (int colGrass = 0; colGrass < cells[rowGrass].length; colGrass++) {
                Cell cell = new Cell(Asset.instance().getSprite(randomGrass(rowGrass, colGrass)), rowGrass * SCALE,
                        colGrass * SCALE);
